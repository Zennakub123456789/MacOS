...
function MacUI:Window(config)
    ...
    if config.ConfigurationSaving and config.ConfigurationSaving.Enabled then
        ...
        self.SaveConfig = function()
            if config.ConfigurationSaving and config.ConfigurationSaving.Enabled then
                local saveData = {
                    __version = 1.1,
                    __elements = {},
                    __custom = self.ConfigData
                }
                
                for flag, element in pairs(self.ConfigElements) do
                    local parser = self.ConfigParser[element.__type]
                    if parser and parser.Save then
                        saveData.__elements[flag] = parser.Save(element)
                    end
                end
                
                SaveConfig(configFileName, saveData)
                print("[MacUI] Config saved:", configFileName .. ".json")
            end
        end
        
        local loadedConfig = LoadConfig(configFileName)
        if loadedConfig then
            if loadedConfig.__version then
                self.ConfigData = loadedConfig.__custom or {}
                
                task.spawn(function()
                    task.wait(0.5)
                    for flag, data in pairs(loadedConfig.__elements or {}) do
                        local element = self.ConfigElements[flag]
                        if element and self.ConfigParser[data.__type] then
                            self.ConfigParser[data.__type].Load(element, data)
                        end
                    end
                    print("[MacUI] Config loaded:", configFileName .. ".json")
                end)
            else
                self.ConfigData = loadedConfig
            end
        end
        ...
        task.spawn(function()
            while ScreenGui and ScreenGui.Parent do
                task.wait(5)
                if next(self.ConfigElements) ~= nil or next(self.ConfigData) ~= nil then
                    self.SaveConfig()
                end
            end
        end)
    ...
    function self:Tab(name, icon)
        ...
        function tab:Toggle(cfg)
            ...
            toggleAPI.Get = function()
                return toggleState
            end
                
            toggleAPI.Destroy = function()
                if cfg.Flag and self.ConfigElements[cfg.Flag] then
                    self.ConfigElements[cfg.Flag] = nil
                end
                holder:Destroy()
            end
            
            if cfg.Flag then
                self.ConfigElements[cfg.Flag] = toggleAPI
            end
...
local function roundValue(val)
    if isDecimal then
        ...
        return math.floor(val)
            ...
            sliderAPI.Set = function(newValue)
            ...
            sliderAPI.SetMin = function(newMin)
                min = newMin
                if value < min then
                    value = min
                    sliderAPI.Value = value
            ...
            sliderAPI.SetMax = function(newMax)
                max = newMax
                if value > max then
                    value = max
                    sliderAPI.Value = value
            ...
            if cfg.Flag then
                self.ConfigElements[cfg.Flag] = sliderAPI
            end
