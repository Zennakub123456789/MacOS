...
local function DeleteConfig(fileName)
    ...
    if isfile(filePath) then
        delfile(filePath)
        print("[MacUI Config] Deleted:", filePath)
        return true
    end
    return false
end
...
local function roundValue(val)
    if isDecimal then
        ...
        function tab:ConfigManager(cfg)
            cfg = cfg or {}
            ...
            local titleLabel = create("TextLabel", {
                ...
                Position = UDim2.new(0, 10, 0, 10),
                BackgroundTransparency = 1,
                Text = cfg.Title or "Configuration Manager",
                TextColor3 = Color3.fromRGB(30, 30, 35),
                Font = Enum.Font.GothamBold,
                TextSize = 14,
                TextXAlignment = Enum.TextXAlignment.Left
            ...
            create("UICorner", { Parent = nameInput, CornerRadius = UDim.new(0, 6) })
            create("UIPadding", { Parent = nameInput, PaddingLeft = UDim.new(0, 8) })
            
            local saveBtn = create("TextButton", {
                Parent = holder,
                Size = UDim2.new(0, 110, 0, 32),
                Position = UDim2.new(1, -120, 0, 45),
                BackgroundColor3 = Color3.fromRGB(52, 199, 89),
                BorderSizePixel = 0,
                Text = "💾 บันทึก",
                TextColor3 = Color3.fromRGB(255, 255, 255),
            ...
            create("UIPadding", { Parent = dropdownBtn, PaddingLeft = UDim.new(0, 8) })
            ...
            local statusLabel = create("TextLabel", {
                ...
                Position = UDim2.new(0, 10, 0, 171),
                BackgroundColor3 = Color3.fromRGB(250, 250, 255),
                BorderSizePixel = 0,
                Text = "💡 วิธีใช้:\n1. พิมพ์ชื่อ Config แล้วกด บันทึก\n2. เลือก Config จาก dropdown\n3. กด โหลด เพื่อใช้งาน Config นั้น",
                TextColor3 = Color3.fromRGB(80, 80, 85),
                Font = Enum.Font.Gotham,
                TextSize = 11,
                TextXAlignment = Enum.TextXAlignment.Left,
            ...
            local function updateDropdownList()
                for _, item in pairs(configListItems) do
                    item:Destroy()
                end
                configListItems = {}
                ...
                if #configs == 0 then
                    local emptyLabel = create("TextLabel", {
                        ...
                        Size = UDim2.new(1, 0, 0, 30),
                        BackgroundTransparency = 1,
                        Text = "ไม่มี Config ที่บันทึกไว้",
                        TextColor3 = Color3.fromRGB(150, 150, 155),
                        Font = Enum.Font.Gotham,
                        TextSize = 12
                    })
                    table.insert(configListItems, emptyLabel)
                else
                    for _, configName in ipairs(configs) do
                        ...
                        itemBtn.MouseEnter:Connect(function()
                            tween(itemBtn, 0.1, { BackgroundColor3 = Color3.fromRGB(245, 245, 250) })
                        end)
                        
                        itemBtn.MouseLeave:Connect(function()
                            tween(itemBtn, 0.1, { BackgroundColor3 = Color3.fromRGB(255, 255, 255) })
                        end)
                        
                        itemBtn.MouseButton1Click:Connect(function()
                            selectedConfigName = configName
                            dropdownBtn.Text = "📋 " .. configName
                            dropdownList.Visible = false
                            holder.Size = UDim2.new(1, -20, 0, 280)
                        end)
                        
                        table.insert(configListItems, itemBtn)
                    end
                end
                
                dropdownList.CanvasSize = UDim2.new(0, 0, 0, #configListItems * 30)
            end
            ...
            dropdownBtn.MouseButton1Click:Connect(function()
                ...
                if dropdownList.Visible then
                    updateDropdownList()
                    local listHeight = math.min(120, #configListItems * 30 + 10)
                    dropdownList.Size = UDim2.new(1, -130, 0, listHeight)
                    holder.Size = UDim2.new(1, -20, 0, 280 + listHeight)
                else
            ...
            saveBtn.MouseEnter:Connect(function()
                tween(saveBtn, 0.15, { BackgroundColor3 = Color3.fromRGB(42, 189, 79) })
            end)
            
            saveBtn.MouseLeave:Connect(function()
                tween(saveBtn, 0.15, { BackgroundColor3 = Color3.fromRGB(52, 199, 89) })
            end)
            ...
            saveBtn.MouseButton1Click:Connect(function()
                ...
                if self.ConfigData then
                    SaveConfig(configName, self.ConfigData)
                    MacUI:Notify({
                        Title = "Config Manager",
                        Content = "บันทึก Config '" .. configName .. "' เรียบร้อย!",
                        Duration = 3
                    })
                    nameInput.Text = ""
                    updateDropdownList()
            ...
            loadBtn.MouseEnter:Connect(function()
                tween(loadBtn, 0.15, { BackgroundColor3 = Color3.new(
                    currentTheme.Accent.R * 0.85,
                    currentTheme.Accent.G * 0.85,
                    currentTheme.Accent.B * 0.85
                )})
            ...
            loadBtn.MouseButton1Click:Connect(function()
                if selectedConfigName == "" then
                    MacUI:Notify({
                        Title = "Config Manager",
                        Content = "กรุณาเลือก Config ที่จะโหลด!",
                        Duration = 3
                    })
                    return
                end
            ...
            deleteBtn.MouseEnter:Connect(function()
                tween(deleteBtn, 0.15, { BackgroundColor3 = Color3.fromRGB(245, 49, 38) })
            end)
            
            deleteBtn.MouseLeave:Connect(function()
                tween(deleteBtn, 0.15, { BackgroundColor3 = Color3.fromRGB(255, 59, 48) })
            end)
            ...
            deleteBtn.MouseButton1Click:Connect(function()
                if selectedConfigName == "" then
                    MacUI:Notify({
                        Title = "Config Manager",
                        Content = "กรุณาเลือก Config ที่จะลบ!",
                        Duration = 3
                    })
                    return
                end
                ...
                MacUI:Dialog({
                    Title = "ยืนยันการลบ",
                    Content = "คุณต้องการลบ Config '" .. selectedConfigName .. "' หรือไม่?",
                    ConfirmText = "ลบ",
                    CancelText = "ยกเลิก",
                    Callback = function(confirmed)
                        if confirmed then
                            if DeleteConfig(selectedConfigName) then
                                MacUI:Notify({
                                    Title = "Config Manager",
                                    Content = "ลบ Config '" .. selectedConfigName .. "' เรียบร้อย!",
                                    Duration = 3
                                })
                                selectedConfigName = ""
                                dropdownBtn.Text = "📋 เลือก Config..."
                                updateDropdownList()
                            end
                        end
                    end
